...
public class UpCycleCommaThread implements Runnable {
	...
	public UpCycleCommaThread(BlockingQueue<ChannelData> down2TmnlQueue, List<String> commAddrs, int upCycle,
			int upDelay) {
	...
	private ByteBuf createGateHead(String commAddr) throws Exception {
		...
		headBuf.writeInt8((byte) (3 & 0xFF));// 报文类型
		headBuf.writeInt8((byte) (0 & 0xFF));// 报文格式版本
		headBuf.writeInt32(Integer.parseInt(commAddr));// 终端通信地址
		// 状态查询报文开始
		headBuf.writeInt16(upCycle);// 报文
		headBuf.writeInt16(upDelay);// 报文
		// 状态查询报文结束
		headBuf.writeInt8((byte) (JNA.CRC8Cal(headBuf.getDataBuffer(), 16) & 0xFF));// CRC8 校验码
		headBuf.writeInt8(Integer.valueOf(ConstantValue.GATE_HEAD_DATA).byteValue());
		headBuf.writeInt8(Integer.valueOf(ConstantValue.GATE_HEAD_DATA).byteValue());
		headBuf.writeInt8(Integer.valueOf(ConstantValue.GATE_HEAD_DATA).byteValue());
